...
import {
  adminOverview, adminListUsers, adminListSquads,
  adminDeleteSquad, adminDeleteUser,
} from '../lib/admin.js';
...
// and users; tapping a row opens its detail page. Delete lives here and on the detail page.
...
const Chevron = () => (
  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="var(--text3)" strokeWidth="2" strokeLinecap="round" style={{ flex: 'none' }}><path d="M9 6l6 6-6 6" /></svg>
);
...
export default function Admin({ getToken, actions }) {
  ...
  const deleteSquad = (sq) => confirm.open({
    title: 'Delete group',
      ...
      {tab === 'groups' && (
        <>
          ...
          <div style={s(card)}>
            ...
            {squads.map((sq, i) => (
              <div key={sq.id} style={s('display:flex;align-items:center;gap:11px;padding:12px 14px' + (i ? ';border-top:1px solid var(--line)' : ''))}>
                <Avatar color={sq.color || '#3a3a3d'} initials={(sq.name || '?').slice(0, 2).toUpperCase()} />
                <div className="ctl" onClick={() => actions?.openAdminGroup(sq.id)} style={s('flex:1;min-width:0')}>
                ...
                <div className="ctl" onClick={() => actions?.openAdminGroup(sq.id)}><Chevron /></div>
              </div>
            ))}
          </div>
        </>
          ...
          <div style={s(card)}>
            ...
            {users.map((u, i) => (
              <div key={u.id} style={s('display:flex;align-items:center;gap:11px;padding:12px 14px' + (i ? ';border-top:1px solid var(--line)' : ''))}>
                ...
                <div className="ctl" onClick={() => actions?.openAdminUser(u.id)} style={s('flex:1;min-width:0')}>
                ...
                <div className="ctl" onClick={() => actions?.openAdminUser(u.id)}><Chevron /></div>
              </div>
            ))}
          </div>
